...
def google_oauth_start(
    ...
    if not settings.google_client_id:
        ...
        raise HTTPException(
            status_code=500, detail="Google OAuth not configured - missing GOOGLE_CLIENT_ID"
        )

    if not settings.google_redirect_uri:
        logger.error("GOOGLE_REDIRECT_URI not configured")
        raise HTTPException(
            status_code=500, detail="Google OAuth not configured - missing GOOGLE_REDIRECT_URI"
        )
...
async def google_oauth_callback(
    ...
    try:
        ...
        existing = (
            db.query(GoogleOAuthToken).filter(GoogleOAuthToken.tenant_id == tenant_id).first()
        )
        ...
        if existing:
            ...
            existing.refresh_token_encrypted = (
                refresh_token_encrypted or existing.refresh_token_encrypted
            )
...
def google_oauth_status(
    ...
    """
    token = db.query(GoogleOAuthToken).filter(GoogleOAuthToken.tenant_id == business_id).first()
